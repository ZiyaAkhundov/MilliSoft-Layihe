// import { username } from './daxilol2.js';
// console.log(username);
// document.querySelector("#navbar_account_p").innerText=username;
// loading
...
function darkmode(){
    ...
    if(JSON.parse(localStorage.getItem(account)).dark=="dark"){
        ...
        document.querySelector(".accounts-div").classList.toggle("dark");
        document.querySelector("#accountsXmark").classList.toggle("white-color");
...
darkmode();
function accounts_check(){
    accountsXmark.addEventListener("click",function(){
        document.querySelector(".accounts-div").classList.toggle("accounts-div-blocks")
    });
    accounts.addEventListener("click",function(event){
        event.preventDefault();
        document.querySelector(".accounts-div").classList.toggle("accounts-div-blocks")
    });
    for(let i=0;i<localStorage.length;i++){
        let a=localStorage.key(i);
        if(JSON.parse(localStorage.getItem(a)).verification=="waiting"){
            let template=`
            <div class="accounts-user">
             ...
             <div class="account-activate">
              <button class="account-accept">Accept</button>
              <button class="account-reject">Reject</button> 
             </div> 
            </div>
            `;
            document.querySelector(".accounts-blocks").innerHTML+=template
        }  
    }
    let accept=document.querySelectorAll(".account-accept");
    accept.forEach(item=>{
        item.addEventListener("click",function(){
            alert("Request accepted");
            let usernamep=item.parentElement.parentElement.querySelector("p").innerText;
            let user={
                username:usernamep,
                password:JSON.parse(localStorage.getItem(usernamep)).password,
                email:JSON.parse(localStorage.getItem(usernamep)).email,
                photo:JSON.parse(localStorage.getItem(usernamep)).photo,
                dark:JSON.parse(localStorage.getItem(usernamep)).dark,
                verification:"verified"
            }
            localStorage.setItem(usernamep,JSON.stringify(user));
        })
    })
    let reject=document.querySelectorAll(".account-reject");
    reject.forEach(item=>{
        item.addEventListener("click",function(){
            alert("Request rejected");
            let p=`<p class="reject-p">Reject</p>`
            item.parentElement.parentElement.querySelector(".account-username").innerHTML+=p;
